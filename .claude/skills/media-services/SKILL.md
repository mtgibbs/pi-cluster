...
- **Mount resilience**: a plain `hard` mount turns a brief NAS stall into a *permanent* freeze (see recovery runbook below). The right fix differs by access mode:
  - **Read-only** PVs (`jellyfin-video-nfs`, `kiwix-zim-nfs`): `soft,timeo=150,retrans=2,nconnect=4` — `soft` makes a stall **error-and-recover** instead of hang (safe — no writes to corrupt). `timeo` lowered 600→150 (15s, fail-fast — a stall becomes a blip not a multi-min freeze) 2026-06-18. Jellyfin soft-mount: 2026-06-15; kiwix brought to standard: 2026-06-18.
