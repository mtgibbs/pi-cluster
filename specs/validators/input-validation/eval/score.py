#!/usr/bin/env python3
"""Score the input-validation validator against its eval set.

Single concern: did the validator correctly call each diff an unvalidated-input
hazard or not? Positive class = "not safe to merge as-is" = {fail, flag};
pass = negative.
...
Validator-output JSON (what the harness emits):
  {"results": [
     {"id": "raw-namespace-to-k8s", "verdict": "fail", "findings": ["..."]},
     ...]}
...
def main():
    ...
    print(f"input-validation eval: {len(cases)} cases — "
          f"PASS={n_pass} FAIL={n_fail} FLAG={n_flag}")
    for c in cases:
        assert c["verdict"] in ("pass", "fail", "flag"), \
            f"{c['id']}: unknown verdict {c['verdict']!r}"
        assert c.get("file") and c.get("diff"), f"{c['id']}: missing file/diff"
    print("  self-check ok: every case has a known verdict + file + diff")
    ...
    for c in cases:
        ...
        print(f"  {mark}{c['id']:<30} exp={exp:<5} pred={pred:<5} [{kind}]")
    ...
    print(f"  [{'PASS' if not missed else 'FAIL'}] every unvalidated sink caught — missed: {missed or 'none'}")
