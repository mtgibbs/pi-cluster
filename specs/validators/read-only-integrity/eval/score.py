#!/usr/bin/env python3
"""Score the read-only-integrity validator against its eval set.

Single concern: did the validator correctly call each diff a read-only tool that
GAINS a write (or not)? Positive class = "not safe to merge as-is" = {fail, flag};
pass = negative (read-only stays read-only, or a writer stays a writer).
...
Validator-output JSON (what the harness emits):
  {"results": [
     {"id": "health-check-patches", "verdict": "fail", "findings": ["..."]},
     ...]}
...
def main():
    ...
    print(f"read-only-integrity eval: {len(cases)} cases — "
    ...
    print(f"  [{'PASS' if not missed else 'FAIL'}] every introduced write caught — missed: {missed or 'none'}")
