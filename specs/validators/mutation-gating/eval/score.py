#!/usr/bin/env python3
"""Score the mutation-gating validator against its eval set.

Single concern: did the validator correctly call each diff a NEW ungated mutating
tool or not? Positive class = "not safe to merge as-is" = {fail, flag}; pass = negative.
...
Validator-output JSON (what the harness emits):
  {"results": [
     {"id": "ungated-delete-pod", "verdict": "fail", "findings": ["..."]},
     ...]}
...
def main():
    ...
    print(f"mutation-gating eval: {len(cases)} cases — "
          f"PASS={n_pass} FAIL={n_fail} FLAG={n_flag}")
    for c in cases:
        assert c.get("verdict") in VERDICTS, f"{c['id']}: unknown verdict {c.get('verdict')!r}"
    ...
    print(f"  [{'PASS' if not missed else 'FAIL'}] every wide-open mutation caught — missed: {missed or 'none'}")
