#!/usr/bin/env python3
"""Score the fail-closed validator against its eval set.

Single concern: did the validator correctly call each diff a fail-OPEN regression
or not? Positive class = "not safe to merge as-is" = {fail, flag}; pass = negative.
...
Validator-output JSON (what the harness emits):
  {"results": [
     {"id": "warn-and-proceed-triggerable", "verdict": "fail",
      "findings": ["..."]},
     ...]}
...
def main():
    ...
    print(f"fail-closed eval: {len(cases)} cases — "
    ...
    print(f"  [{'PASS' if not missed else 'FAIL'}] every fail-open caught — missed: {missed or 'none'}")
