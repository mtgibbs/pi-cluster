#!/usr/bin/env python3
"""Score the secret-hygiene validator against its eval set.

Single concern: did the validator correctly call each diff a secret LEAK or not?
Positive class = "not safe to merge as-is" = {fail, flag}; pass = negative.
...
Validator-output JSON (what the harness emits):
  {"results": [
     {"id": "drop-secretkeyref-branch", "verdict": "fail",
      "findings": ["..."]},
     ...]}
...
def main():
    ...
    print(f"  [{'PASS' if not missed else 'FAIL'}] every leak caught — missed: {missed or 'none'}")
