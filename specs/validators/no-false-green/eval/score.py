#!/usr/bin/env python3
"""Score the no-false-green validator against its eval set.

Single concern: did the validator correctly call each diff a status/reader tool that
can report success/healthy/ok/empty on a real FAILURE (or not)? Positive class =
"not safe to merge as-is" = {fail, flag}; pass = negative (failures surface honestly,
or the change removes a false-green / does not touch a failure path).
...
Validator-output JSON (what the harness emits):
  {"results": [
     {"id": "swallow-into-empty-array", "verdict": "fail", "findings": ["..."]},
     ...]}
...
def main():
    ...
    print(f"no-false-green eval: {len(cases)} cases — "
    ...
    print("  self-check ok: unique ids, every case has a known verdict + file + diff")
    ...
    for c in cases:
        ...
        print(f"  {mark}{c['id']:<32} exp={exp:<5} pred={pred:<5} [{kind}]")
    ...
    print("\n--- DoD checks ---")
    print(f"  [{'PASS' if not fp_ids else 'FAIL'}] no false-blocks on honest/safe diffs — {fp_ids or 'none'}")
    print(f"  [{'PASS' if not missed else 'FAIL'}] every false-green caught — missed: {missed or 'none'}")
