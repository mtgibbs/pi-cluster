#!/usr/bin/env python3
"""Score the output-bounds validator against its eval set.

Single concern: did the validator correctly call each diff a tool that can return
UNBOUNDED size-scaling output (or not)? Positive class = "not safe to merge as-is" =
{fail, flag}; pass = negative (output is bounded / fixed-shape / a small infra list, or
the change adds/tightens a bound).
...
Validator-output JSON (what the harness emits):
  {"results": [
     {"id": "remove-slice-cap", "verdict": "fail", "findings": ["..."]},
     ...]}
...
def main():
    ...
    print(f"output-bounds eval: {len(cases)} cases — "
    ...
    print("\n--- DoD checks ---")
    print(f"  [{'PASS' if not fp_ids else 'FAIL'}] no false-blocks on bounded/safe diffs — {fp_ids or 'none'}")
    print(f"  [{'PASS' if not missed else 'FAIL'}] every unbounded path caught — missed: {missed or 'none'}")
