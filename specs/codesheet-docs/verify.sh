...
check AC2  "names the codesheet generator"         grep -q 'gen-codesheet\.mjs' "$R"
check AC2b "sources it from mtgibbs/harness"       grep -q 'mtgibbs/harness' "$R"
...
check AC6  "mentions the build loop by name"       grep -q 'ralph-build\.sh' "$R"
...
echo "VERIFY: $((13 - fails))/13 checks passed"
[ "$fails" -eq 0 ]
