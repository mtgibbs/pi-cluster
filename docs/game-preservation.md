...
Admin account created + first login confirmed working (gotcha: RomM's login form takes
**username**, not email — `mtgibbs`, not the email address used at signup). |
...
| 5 | Add RomM data to backup scope | C → CO | **DONE 2026-07-09**: `mariadb-backup` CronJob (node-independent, Sundays 3:45, `--single-transaction` dump → `{date}/romm/`) + `romm_romm-config` added to `media-backup` (both RomM pods landed on pi5-worker-1); `romm-redis-data` = regenerable cache, skipped. Details + restore procedure in `.claude/skills/backup-ops/SKILL.md`. **Still open (H):** decide QNAP-side snapshot/2nd-copy for `/share/cluster/games` — the dated `/backups` tree and the library live on the same box. |
