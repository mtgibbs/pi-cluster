...
2. **Build + publish the image:** `ghcr.io/mtgibbs/new-horizons:0.1.0` (Dockerfile + CI live in the
   app repo). ⚠️ **Keep the GHCR package PRIVATE** — the image bakes in `profile/`
   (`base-resume.md`, `accomplishments-record.md`), i.e. real PII. Packages are created private by
   default, so this is a "don't touch it" rather than a step. An earlier draft of this line said to
   run `gh api PATCH … -f visibility=public`; that was wrong — do not. The cluster pulls it with the
   shared read-only `ghcr-pull-secret`, so private costs us nothing.
