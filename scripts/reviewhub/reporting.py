...
def render_report(summaries, files, version):
    ...
    if blockers:
        out += ["> [!CAUTION]",
                ...
                "Each validator's own check is the gate.", ""]
        for s in blockers:
    ...
    out.append(f"<sub>reviewed {filetxt}{more} · the merge gate is each validator's own "
               f"check above · review-hub {version}</sub>")
    return "\n".join(out)
