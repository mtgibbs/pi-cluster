#!/usr/bin/env python3
"""read-only-integrity — a single-concern validator.

ONE question: does a change to a READ-ONLY tool in pi-cluster-mcp (or a shared
helper it calls) INTRODUCE a write / mutation / exec-of-a-mutating-command /
delete path where there was none? Nothing else. Reuses the model backend from
triggerable_judge; its own prompt, parser, and eval set
(specs/validators/read-only-integrity/eval/).

  # measure it against the ruler
  read_only_integrity.py --eval --backend litellm --repeat 5 --out /tmp/roi.json --score
  # one case / prompt only
  read_only_integrity.py --id health-check-patches [--dry-run]
"""
import argparse
import json
import os
...
EVAL = REPO_ROOT / "specs/validators/read-only-integrity/eval/expected.yaml"
SCORER = REPO_ROOT / "specs/validators/read-only-integrity/eval/score.py"
BEGIN, END = tj.BEGIN, tj.END

# The prompt template lives next to its eval set (specs/validators/read-only-integrity/
...
PROMPT_TEMPLATE = (REPO_ROOT / "specs/validators/read-only-integrity/contract.md").read_text()


def build_prompt(changes):
    """changes = [(file, unified_diff), …] — judged TOGETHER so the model has
    cross-file context (a read-only tool's write may be introduced in a shared
    helper in a DIFFERENT file in the same change set)."""
...
def _render_pr(res):
    head = tj.comment_marker("read-only-integrity") + "\n## 👁️ read-only-integrity\n\n" + (
        "🚫 A read-only tool may have **gained a write/mutation path** — human review required.\n\n"
        if res["block"] else "✅ No read-only tool gains a write — observers stay observers.\n\n")
...
class ReadOnlyIntegrityValidator:
    """Single-concern: does a read-only tool (or a helper it calls) gain a write?"""
    ...
    def review(self, forge, reps, timeout, model, raw_dir):
        ...
        # model see a read-only tool whose new write is introduced in a shared helper.
        runs = []
        for r in range(reps):
...
def main():
    ...
    raw_dir = args.raw_dir or tempfile.mkdtemp(prefix="read-only-integrity-")
    os.makedirs(raw_dir, exist_ok=True)
    reps = max(1, args.repeat)
    print(f"read-only-integrity: {len(cases)} case(s) x{reps}; backend={args.backend} "
