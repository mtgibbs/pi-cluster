#!/usr/bin/env python3
"""no-false-green — a single-concern validator.

ONE question: does a change make a STATUS / DIAGNOSTIC / READER tool in pi-cluster-mcp
report success / healthy / ok / empty-but-fine when the underlying operation actually
FAILED, ERRORED, or was SKIPPED? Nothing else. Reuses the model backend from
triggerable_judge; its own prompt, parser, and eval set
(specs/validators/no-false-green/eval/).

  # measure it against the ruler
  no_false_green.py --eval --backend litellm --repeat 5 --out /tmp/nfg.json --score
  # one case / prompt only
  no_false_green.py --id swallow-into-empty-array [--dry-run]
"""
import argparse
import json
import os
...
EVAL = REPO_ROOT / "specs/validators/no-false-green/eval/expected.yaml"
SCORER = REPO_ROOT / "specs/validators/no-false-green/eval/score.py"
BEGIN, END = tj.BEGIN, tj.END

# The prompt template lives next to its eval set (specs/validators/no-false-green/
...
PROMPT_TEMPLATE = (REPO_ROOT / "specs/validators/no-false-green/contract.md").read_text()


def build_prompt(changes):
    """changes = [(file, unified_diff), …] — judged TOGETHER so the model has
    cross-file context (a reader's health may be derived from a helper whose failure
    behavior lives in a DIFFERENT file in the same change set)."""
    blocks = [f"--- FILE: {file} ---\n{diff}" for file, diff in changes]
    header = (f"THE CHANGES UNDER REVIEW — {len(changes)} changed tool/helper "
...
def aggregate(runs):
    """Majority-to-escalate: block only when MOST of the votes escalate. (error counts
    as escalating; a minority of strays is denoised.) Positive class = {fail, flag}."""
...
def _render_pr(res):
    head = tj.comment_marker("no-false-green") + "\n## 🚦 no-false-green\n\n" + (
        "🚫 A status/reader tool may **report green on a real failure** — human review required.\n\n"
        if res["block"] else "✅ Failures surface honestly — no green light masks an error.\n\n")
...
class NoFalseGreenValidator:
    """Single-concern: can a status/reader tool report green when the check failed?"""

    # ---- routing config: which repo + which files this validator runs on ----
    name = "no-false-green"
    concern = "Can a status/reader tool report healthy/ok/empty when the underlying check actually failed or was skipped?"
    ...
    def review(self, forge, reps, timeout, model, raw_dir):
        ...
        if not changes:
            return [], False, None
        # Judge ALL changed tool/helper files TOGETHER — cross-file context lets the
        # model trace a reader's health back to a helper's failure behavior.
        runs = []
        for r in range(reps):
...
def main():
    ...
    raw_dir = args.raw_dir or tempfile.mkdtemp(prefix="no-false-green-")
    os.makedirs(raw_dir, exist_ok=True)
    reps = max(1, args.repeat)
    print(f"no-false-green: {len(cases)} case(s) x{reps}; backend={args.backend} "
