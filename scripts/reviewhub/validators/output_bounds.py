#!/usr/bin/env python3
"""output-bounds — a single-concern validator.

ONE question: does a change let a tool in pi-cluster-mcp return UNBOUNDED output —
output whose size scales with data volume (logs, exec/command stdout, query/queue/history
records, search results, large/cluster-wide resource enumerations) returned with no
limit / cap / slice / page size — that could blow the LLM caller's context? Nothing else.
Reuses the model backend from triggerable_judge; its own prompt, parser, and eval set
(specs/validators/output-bounds/eval/).

  # measure it against the ruler
  output_bounds.py --eval --backend litellm --repeat 5 --out /tmp/ob.json --score
  # one case / prompt only
  output_bounds.py --id remove-slice-cap [--dry-run]
"""
import argparse
import json
import os
...
EVAL = REPO_ROOT / "specs/validators/output-bounds/eval/expected.yaml"
SCORER = REPO_ROOT / "specs/validators/output-bounds/eval/score.py"
BEGIN, END = tj.BEGIN, tj.END

# The prompt template lives next to its eval set (specs/validators/output-bounds/
...
PROMPT_TEMPLATE = (REPO_ROOT / "specs/validators/output-bounds/contract.md").read_text()


def build_prompt(changes):
    """changes = [(file, unified_diff), …] — judged TOGETHER so the model has
    cross-file context (a tool's output bound may live in a shared helper in a
    DIFFERENT file in the same change set)."""
...
def _render_pr(res):
    head = tj.comment_marker("output-bounds") + "\n## 📏 output-bounds\n\n" + (
        "🚫 A tool may return **unbounded output** that blows the caller's context — human review required.\n\n"
        if res["block"] else "✅ Output is bounded — no unbounded list/log/stdout returned.\n\n")
...
class OutputBoundsValidator:
    """Single-concern: can a tool return unbounded size-scaling output (no cap)?"""

    # ---- routing config: which repo + which files this validator runs on ----
    name = "output-bounds"
    concern = "Does a tool return unbounded list/log/stdout output (no limit/cap/slice) that could blow the caller's context?"
    ...
    def review(self, forge, reps, timeout, model, raw_dir):
        ...
        # model trace a tool's output bound into a shared helper.
        runs = []
        for r in range(reps):
...
def main():
    ...
    raw_dir = args.raw_dir or tempfile.mkdtemp(prefix="output-bounds-")
    os.makedirs(raw_dir, exist_ok=True)
    reps = max(1, args.repeat)
    print(f"output-bounds: {len(cases)} case(s) x{reps}; backend={args.backend} "
