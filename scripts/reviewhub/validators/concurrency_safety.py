#!/usr/bin/env python3
"""concurrency-safety — a single-concern validator.

ONE question: does a MUTATING tool in pi-cluster-mcp tolerate CONCURRENT /
OVERLAPPING invocation? Reference is Guard A in backups.ts (refuse if a run is
already active: scheduledActive + manualActive > 0 -> AlreadyRunningError).
Nothing else — not style, not perf, not the triggerable label or deploy allowlist
(those belong to gate-regression). Reuses the model backend from triggerable_judge;
its own prompt, parser, and eval set (specs/validators/concurrency-safety/eval/).

  # measure it against the ruler
  concurrency_safety.py --eval --backend litellm --repeat 5 --out /tmp/cs.json --score
  # one case / prompt only
  concurrency_safety.py --id remove-guard-a [--dry-run]
"""
import argparse
import json
import os
...
EVAL = REPO_ROOT / "specs/validators/concurrency-safety/eval/expected.yaml"
SCORER = REPO_ROOT / "specs/validators/concurrency-safety/eval/score.py"
BEGIN, END = tj.BEGIN, tj.END

# The prompt template lives next to its eval set (specs/validators/concurrency-safety/
...
PROMPT_TEMPLATE = (REPO_ROOT / "specs/validators/concurrency-safety/contract.md").read_text()


def build_prompt(changes):
    """changes = [(file, unified_diff), …] — judged TOGETHER so the model has
    cross-file context (a guard helper defined in one file is often called from a
    mutating tool in another)."""
...
def aggregate(runs):
    ...
    on the eval, the gap is clean — a real violation is unanimous (5/5), while a
    safe-but-subtle change the model occasionally misreads tops out at 2/5. Strict
    majority (>=3 of 5) sits in that gap: every violation blocks, no safe change
...
def _render_pr(res):
    head = tj.comment_marker("concurrency-safety") + "\n## 🔒 concurrency-safety\n\n" + (
        "🚫 A change may be **unsafe under concurrent invocation** — human review required.\n\n"
        if res["block"] else "✅ No concurrency-safety guard is weakened by these changes.\n\n")
...
class ConcurrencySafetyValidator:
    """Single-concern: does a mutating tool in pi-cluster-mcp tolerate concurrent /
    overlapping invocation (Guard A and friends)?"""
    ...
    def review(self, forge, reps, timeout, model, raw_dir):
        ...
        if not changes:
            return [], False, None
        # Judge ALL changed mutation-relevant files TOGETHER — cross-file context lets
        # the model see a guard helper defined in one file and called from a tool in
        # another (a relocated guard is wired-up, not removed).
        runs = []
        for r in range(reps):
...
def main():
    ...
    raw_dir = args.raw_dir or tempfile.mkdtemp(prefix="concurrency-safety-")
    os.makedirs(raw_dir, exist_ok=True)
    reps = max(1, args.repeat)
    print(f"concurrency-safety: {len(cases)} case(s) x{reps}; backend={args.backend} "
