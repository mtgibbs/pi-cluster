#!/usr/bin/env python3
"""input-validation — a single-concern validator.

ONE question: does a PR diff let a USER-SUPPLIED tool parameter reach a dangerous
sink (k8s API / SSH / shell-string exec / outbound URL) in pi-cluster-mcp WITHOUT
validation? Nothing else. Reuses the model backend from triggerable_judge; its own
prompt, parser, and eval set (specs/validators/input-validation/eval/).

  # measure it against the ruler
  input_validation.py --eval --backend litellm --repeat 5 --out /tmp/iv.json --score
  # one case / prompt only
  input_validation.py --id raw-namespace-to-k8s [--dry-run]
"""
import argparse
import json
import os
...
EVAL = REPO_ROOT / "specs/validators/input-validation/eval/expected.yaml"
SCORER = REPO_ROOT / "specs/validators/input-validation/eval/score.py"
BEGIN, END = tj.BEGIN, tj.END

# The prompt template lives next to its eval set (specs/validators/input-validation/
...
PROMPT_TEMPLATE = (REPO_ROOT / "specs/validators/input-validation/contract.md").read_text()


def build_prompt(changes):
    """changes = [(file, unified_diff), …] — judged TOGETHER so the model has
    cross-file context (a validator defined in one file is often called in another,
    e.g. validateNodeName in node-validation.ts, sanitizePath in synology.ts)."""
    blocks = [f"--- FILE: {file} ---\n{diff}" for file, diff in changes]
    header = (f"THE CHANGES UNDER REVIEW — {len(changes)} changed input-relevant "
...
def _render_pr(res):
    head = tj.comment_marker("input-validation") + "\n## 🧪 input-validation\n\n" + (
        "🚫 A user-supplied input may reach a sink **without validation** — human review required.\n\n"
        if res["block"] else "✅ All tool inputs are validated before reaching a sink.\n\n")
...
class InputValidationValidator:
    """Single-concern: does a PR diff let a user param reach a sink unvalidated?"""
    ...
    def review(self, forge, reps, timeout, model, raw_dir):
        ...
        if not changes:
            return [], False, None
        # Judge ALL changed input-relevant files TOGETHER — cross-file context lets
        # the model see a validator defined in one file (validateNodeName,
        # sanitizePath) and called in another (the tool that hits the sink).
        runs = []
        for r in range(reps):
...
def main():
    ...
    raw_dir = args.raw_dir or tempfile.mkdtemp(prefix="input-validation-")
    os.makedirs(raw_dir, exist_ok=True)
    reps = max(1, args.repeat)
    print(f"input-validation: {len(cases)} case(s) x{reps}; backend={args.backend} "
    ...
    for c in cases:
        ...
        print(f"  {c['id']:<30} -> {res['verdict']:<5}{seq}", file=sys.stderr)
