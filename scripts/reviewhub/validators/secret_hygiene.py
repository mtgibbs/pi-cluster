#!/usr/bin/env python3
"""secret-hygiene — a single-concern validator.

ONE question: does a PR diff make a pi-cluster-mcp tool LEAK a secret VALUE in its
OUTPUT? Nothing else. MCP tools return their result straight to the caller, so that
result must never carry plaintext secret material (raw env values behind a secretKeyRef,
a Secret's decoded .data, tokens/passwords/apiKeys, kubeconfig). It upholds the
sanitizeEnvVars/sanitizeEnvFrom/sanitizeVolume redaction pattern. Reuses the model
backend from triggerable_judge; its own prompt, parser, and eval set
(specs/validators/secret-hygiene/eval/).

  # measure it against the ruler
  secret-hygiene.py --eval --backend litellm --repeat 5 --out /tmp/sh.json --score
  # one case / prompt only
  secret-hygiene.py --id drop-secretkeyref-branch [--dry-run]
"""
import argparse
import json
import os
...
EVAL = REPO_ROOT / "specs/validators/secret-hygiene/eval/expected.yaml"
SCORER = REPO_ROOT / "specs/validators/secret-hygiene/eval/score.py"
BEGIN, END = tj.BEGIN, tj.END

# The prompt template lives next to its eval set (specs/validators/secret-hygiene/
...
PROMPT_TEMPLATE = (REPO_ROOT / "specs/validators/secret-hygiene/contract.md").read_text()


def build_prompt(changes):
    """changes = [(file, unified_diff), …] — judged TOGETHER so the model has
    cross-file context (a sanitizer is defined in one file and called where a tool
    builds its response in another)."""
    blocks = [f"--- FILE: {file} ---\n{diff}" for file, diff in changes]
    header = (f"THE CHANGES UNDER REVIEW — {len(changes)} changed output-relevant "
...
def aggregate(runs):
    ...
    on the eval, the gap is clean — a real leak is unanimous (5/5), while a
    safe-but-subtle change the model occasionally misreads tops out at 2/5. Strict
    majority (>=3 of 5) sits in that gap: every leak blocks, no safe change
...
def _render_pr(res):
    head = tj.comment_marker("secret-hygiene") + "\n## 🔑 secret-hygiene\n\n" + (
        "🚫 A change may **leak a secret value in tool output** — human review required.\n\n"
        if res["block"] else "✅ No secret value is leaked in tool output by these changes.\n\n")
...
class SecretHygieneValidator:
    """Single-concern: does a PR diff make a pi-cluster-mcp tool LEAK a secret value
    in its OUTPUT?"""
    ...
    def review(self, forge, reps, timeout, model, raw_dir):
        ...
        if not changes:
            return [], False, None
        # Judge ALL changed output-relevant files TOGETHER — cross-file context lets
        # the model see a sanitizer defined in one file and called from a tool in another.
        runs = []
        for r in range(reps):
...
def main():
    ...
    raw_dir = args.raw_dir or tempfile.mkdtemp(prefix="secret-hygiene-")
    os.makedirs(raw_dir, exist_ok=True)
    reps = max(1, args.repeat)
    print(f"secret-hygiene: {len(cases)} case(s) x{reps}; backend={args.backend} "
