...
// Arm C: serena MCP server registered for this run only, via OPENCODE_CONFIG
// (verified 2026-07-11: the env-pointed config MERGES with the global one, so
// provider/model/homelab MCPs are inherited — only serena is added).
// Serena indexes per project into <ROOT>/.serena/cache; the first trial pays
// the indexing + TS-language-server download, so warm before comparing.
let serenaEnv = {};
if (ARM === "C") {
  try { execFileSync("uvx", ["--version"], { stdio: "ignore" }); }
  catch {
    console.error("arm C needs uvx (uv) in this container — not baked yet.");
    console.error("Image-bake ask + runbook: scripts/token-bench/serena-prep.md");
    process.exit(2);
  }
  const SERENA_REF = opt("serena-ref", "v1.5.3"); // research-doc version; override as it moves
  // Pre-seed project config so node_modules never enters the index (the #1
  // serena ops gotcha) — only if the target repo has none.
  const projYml = join(ROOT, ".serena/project.yml");
  if (!existsSync(projYml)) {
    mkdirSync(dirname(projYml), { recursive: true });
    writeFileSync(projYml, [
      "# generated by token-bench arm C — safe to delete; serena regenerates defaults",
      "language: typescript",
      "ignore_all_files_in_gitignore: true",
      'ignored_paths: ["node_modules", ".next", "public", "results"]',
      "read_only: true", // bench is Q&A only; keep symbol-editing tools off
    ].join("\n") + "\n");
    console.error(`arm C: seeded ${projYml}`);
  }
  const cfgPath = join(tmpdir(), `token-bench-serena-${process.pid}.json`);
  writeFileSync(cfgPath, JSON.stringify({
    $schema: "https://opencode.ai/config.json",
    mcp: {
      serena: {
        type: "local",
        command: ["uvx", "--from", `git+https://github.com/oraios/serena@${SERENA_REF}`,
          "serena", "start-mcp-server", "--context", "ide-assistant", "--project", ROOT],
        enabled: true,
      },
    },
  }, null, 2));
  serenaEnv = { OPENCODE_CONFIG: cfgPath };
}
...
const SYMBOL_BLOCK = (s) =>
  ...
  "\n<symbol-graph>\n" + s + "</symbol-graph>\n\n";
// Arm C gets a tool hint instead of an injected sheet — the symmetric
// treatment (every non-baseline arm is told how to use its aid).
const SERENA_INSTR =
  "This repository is indexed by the serena MCP server. Prefer its symbol tools " +
...
function runOnce(prompt) {
  return new Promise((resolve) => {
    ...
    const child = spawn("opencode", ["run", prompt], {
      ...
      env: { ...process.env, PWD: ROOT, ...serenaEnv },
    });
